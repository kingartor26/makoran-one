...
export class DatabaseService {
  ...
  private seedDefaultData(): void {
    ...
    if (!this.queryOne('SELECT id FROM attendance LIMIT 1')) {
      this.run(`
    ...
    if (!this.queryOne('SELECT id FROM facility_devices LIMIT 1')) {
    ...
    if (!this.queryOne('SELECT id FROM ecommerce_products LIMIT 1')) {
      const products = [
        [
          'prod-01',
